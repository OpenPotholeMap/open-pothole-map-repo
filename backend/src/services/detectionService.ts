  ...
  private async initializeGCS(): Promise<void> {
    try {
      ...
      if (isAccessible) {
        ...
        console.warn('⚠️  Google Cloud Storage bucket access failed - images will use placeholder URLs');
      }
    } catch (error) {
      console.error('🚨 GCS initialization error:', error);
      console.warn('⚠️  Continuing without GCS - images will use placeholder URLs');
    }
  }
  ...
  async savePothole(detection: PotholeDetection): Promise<string | null> {
    try {
      ...
      if (nearbyPothole) {
        ...
        try {
          newImageUrl = await cloudStorageService.uploadImage(detection.imageBuffer, fileName);
      ...
      try {
        ...
        console.error('🚨 Failed to upload image to GCS, using placeholder:', uploadError);
